...
const AllComplaints = () => {
  ...
  const fetchComplaints = async () => {
    ...
    try {
      ...
      const superAdminSnap = await get(child(dbRef, `users/${currentUser.uid}`));
  ...
  return (
    <div className="p-6 max-w-full overflow-x-auto">
      <h2 className="text-3xl font-semibold mb-6 text-gray-800">All Student Complaints</h2>
      {complaints.length === 0 ? (
        ...
        <table className="min-w-[900px] w-full border-collapse border border-gray-300 text-left">
          <thead className="bg-gray-100 sticky top-0">
            <tr>
              {['Name', 'Roll', 'Dept.', 'Email', 'Subject', 'Description', 'Priority', 'Date', 'Status', 'Actions'].map(
                (head) => (
                  <th
                    key={head}
          ...
          <tbody>
            {complaints.map((comp, idx) => (
              ...
              >
                <td className="border border-gray-300 px-3 py-1 max-w-[130px] truncate">{comp.name}</td>
                <td className="border border-gray-300 px-3 py-1">{comp.roll}</td>
                <td className="border border-gray-300 px-3 py-1">{comp.department}</td>
                <td className="border border-gray-300 px-3 py-1 max-w-[180px] truncate">{comp.email}</td>
                <td className="border border-gray-300 px-3 py-1 max-w-[160px] truncate">{comp.subject}</td>
                <td
                ...
                <td className="border border-gray-300 px-3 py-1">{comp.priority}</td>
                <td className="border border-gray-300 px-3 py-1">{comp.date}</td>
                <td className="border border-gray-300 px-3 py-1">
                  <select
                    value={comp.status}
                    onChange={(e) =>
                      handleStatusChange(comp.userId, comp.complaintId, e.target.value)
                    }
